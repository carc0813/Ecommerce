...
const preloadData = async () => {
   ...
   if (existingCategories.length < 4) {
     console.log("⚠️ Faltan categorías, insertando...");
     await Category.bulkCreate(
       ["Shirts", "Pants", "Hoodies", "Hats"].map(name => ({ name })),
       { ignoreDuplicates: true } // Evita errores si ya existen
     );
   }
 
   // **Volver a traer todas las categorías después de insertarlas**
   ...
   console.log("📂 Categorías disponibles en la BD:", categoryMap);
  // Crear productos
  const products = await Product.bulkCreate([
    {
      title: "Men’s Chill Crew Neck Sweatshirt",
      description: `Introducing the Tesla Chill Collection. The Men’s Chill Crew Neck Sweatshirt has 
        a premium, heavyweight exterior and soft fleece interior for comfort in any season. 
        The sweatshirt features a subtle thermoplastic polyurethane T logo on the chest and a 
        Tesla wordmark below the back collar. Made from 60% cotton and 40% recycled polyester.`,
    ...
    },
    {
      ...
      description: `The Men's Quilted Shirt Jacket features a uniquely fit, quilted design for warmth and 
        mobility in cold weather seasons. With an overall street-smart aesthetic, the jacket features subtle 
        silicone injected Tesla logos below the back collar and on the right sleeve, as well as custom matte 
        metal zipper pulls. Made from 87% nylon and 13% polyurethane.`,
    ...
    },
    {
      ...
      description: `Introducing the Tesla Raven Collection. The Men's Raven Lightweight Zip Up Bomber 
        has a premium, modern silhouette made from a sustainable bamboo cotton blend for versatility in any season. 
        The hoodie features subtle thermoplastic polyurethane Tesla logos on the left chest and below the back collar, 
        a concealed chest pocket with custom matte zipper pulls and a french terry interior. Made from 70% bamboo and 30% cotton.`,
    ...
    },
    {
      ...
      description: `Introducing the Tesla Turbine Collection. Designed for style, comfort and everyday lifestyle, 
        the Men's Turbine Long Sleeve Tee features a subtle, water-based T logo on the left chest and our Tesla 
        wordmark below the back collar. The lightweight material is double-dyed, creating a soft, casual style for 
        ideal wear in any season. Made from 50% cotton and 50% polyester.`,
    ...
    },
    {
      ...
      description: `Introducing the Tesla Turbine Collection. Designed for style, comfort and everyday lifestyle, 
        the Men's Turbine Short Sleeve Tee features a subtle, water-based Tesla wordmark across the chest and our T logo 
        below the back collar. The lightweight material is double-dyed, creating a soft, casual style for ideal wear in 
        any season. Made from 50% cotton and 50% polyester.`,
    ...
    },
    {
      ...
      description: `Designed for comfort, the Cybertruck Owl Tee is made from 100% cotton and features 
        our signature Cybertruck icon on the back.`,
  ...
  // **Relacionar productos con categorías correctamente**
  async function assignCategoryByType(products, categoryMap) {
    await Promise.all(
      products.map(async (product) => {
        console.log("Producto:", product); // Depuración para ver la estructura
        const category = categoryMap[product.type];
        if (category) {
          await product.addCategories([category]);
        } else {
          console.warn(`No se encontró categoría para el type: ${product.type}`);
        }
      })
    );
    console.log("🚀 Categorías asignadas según el type de cada producto");
  }
  
  await assignCategoryByType(products, categoryMap);
};
